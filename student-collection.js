/* RATHOD HUB Question Archive: app allowlist only for Daily 9 PM and NEET 720. */
(function(){
'use strict';
...
function remember(k){window.rhArchiveSeen.add(k);try{localStorage.setItem('rh_archive_seen',JSON.stringify([...window.rhArchiveSeen].slice(-800)))}catch(e){}}
function cleanOptions(list){return[...(list||[])].map(x=>{if(x&&typeof x==='object'&&!x.textContent)return String(x.text??x.label??x.value??'');return String(x?.textContent||x||'')}).map(x=>x.replace(/^[A-D][.)]\s*/,'').trim()).filter(Boolean).slice(0,4)}
