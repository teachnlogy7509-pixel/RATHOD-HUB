...
/* Email + mobile OTP authentication with SMS/WhatsApp delivery selection. */
(function(){
'use strict';
...
function addPhoneAuth(){
  ...
  wrap.innerHTML='<div class="flex items-center gap-2 my-3"><div class="h-px flex-1 bg-white/10"></div><span class="text-[10px] text-slate-500 font-bold">OR</span><div class="h-px flex-1 bg-white/10"></div></div><button id="rh-phone-start" type="button" class="w-full rounded-xl border border-cyan-400/30 bg-cyan-500/10 px-4 py-3 text-sm font-black text-cyan-200 hover:bg-cyan-500/20">📱 Continue with Mobile OTP</button><div id="rh-phone-panel" class="hidden mt-3 rounded-2xl border border-cyan-400/20 bg-slate-950/60 p-3"><p class="text-[11px] text-slate-400 mb-2">Mobile number डालें और OTP delivery चुनें.</p><div class="grid grid-cols-2 gap-2 mb-2"><button id="rh-channel-sms" type="button" class="rounded-xl border border-cyan-400/50 bg-cyan-500/15 px-3 py-2 text-xs font-black text-cyan-100">💬 SMS</button><button id="rh-channel-whatsapp" type="button" class="rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-xs font-black text-slate-300">🟢 WhatsApp</button></div><div class="flex gap-2"><input id="rh-phone-input" type="tel" inputmode="tel" autocomplete="tel" placeholder="+91 9876543210" class="rh-mobile-input min-w-0 flex-1"><button id="rh-phone-send" type="button" class="shrink-0 rounded-xl bg-cyan-600 px-3 py-2 text-xs font-black">Send OTP</button></div><div id="rh-phone-verify" class="hidden mt-2"><div class="flex gap-2"><input id="rh-phone-otp" type="text" inputmode="numeric" autocomplete="one-time-code" maxlength="6" placeholder="6-digit OTP" class="rh-mobile-input min-w-0 flex-1 text-center tracking-[.3em]"><button id="rh-phone-check" type="button" class="shrink-0 rounded-xl bg-emerald-600 px-3 py-2 text-xs font-black">Verify</button></div><button id="rh-phone-resend" type="button" class="mt-2 text-[10px] text-cyan-300 underline">Resend OTP</button><p id="rh-phone-status" class="mt-2 text-[10px] text-slate-500"></p></div></div>';
  if(anchor&&anchor.parentNode)anchor.parentNode.insertBefore(wrap,anchor);else byId('auth-form').appendChild(wrap);
  var pending='',delivery='sms';
  function panel(open){byId('rh-phone-panel')?.classList.toggle('hidden',!open)}
  function setDelivery(next){delivery=next==='whatsapp'?'whatsapp':'sms';var sms=byId('rh-channel-sms'),wa=byId('rh-channel-whatsapp');sms.className=delivery==='sms'?'rounded-xl border border-cyan-400/50 bg-cyan-500/15 px-3 py-2 text-xs font-black text-cyan-100':'rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-xs font-black text-slate-300';wa.className=delivery==='whatsapp'?'rounded-xl border border-emerald-400/50 bg-emerald-500/15 px-3 py-2 text-xs font-black text-emerald-100':'rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-xs font-black text-slate-300'}
  async function send(){
    try{
      ...
      var options={shouldCreateUser:true,channel:delivery};if(name)options.data={name:name};
      var r=await window.db.auth.signInWithOtp({phone:pending,options:options});
      if(r.error)throw r.error;
      panel(true);byId('rh-phone-verify').classList.remove('hidden');byId('rh-phone-status').textContent=(delivery==='whatsapp'?'WhatsApp':'SMS')+' OTP sent to '+pending+'. Resend 60 seconds बाद करें.';byId('rh-phone-otp').focus();toastAuth('✅ '+(delivery==='whatsapp'?'WhatsApp':'SMS')+' OTP भेज दिया गया है');
    }catch(e){toastAuth(e.message||'OTP भेजा नहीं जा सका',false)}
  }
  async function verify(){
    try{
      ...
      if(r.data?.session){window.session=r.data.session;window.user=r.data.user;if(typeof window.setAppScreenVisibility==='function')window.setAppScreenVisibility(true);if(typeof window.boot==='function')await window.boot(r.data.session)}
      toastAuth('✅ Mobile verified. RATHOD HUB खुल रहा है.');
    }catch(e){toastAuth(e.message||'OTP verify नहीं हुआ',false)}
  }
  byId('rh-phone-start').onclick=function(){panel(true);byId('rh-phone-input').focus()};byId('rh-channel-sms').onclick=function(){setDelivery('sms')};byId('rh-channel-whatsapp').onclick=function(){setDelivery('whatsapp')};byId('rh-phone-send').onclick=send;byId('rh-phone-check').onclick=verify;byId('rh-phone-resend').onclick=send;setDelivery('sms');
  return true;
}
