...
function movePdfToAdmin(){
 ...
 if(role()==='admin'&&host){lab.classList.remove('hidden');if(lab.parentElement!==host){const title=document.createElement('div');title.id='admin-pdf-quiz-title';title.className='rounded-2xl border border-amber-400/25 bg-amber-500/5 p-4';title.innerHTML='<b class="text-amber-300">📄 Admin PDF → Live Quiz</b><p class="mt-1 text-[10px] text-slate-400">PDF upload aur quiz generation केवल Admin Panel से नियंत्रित होगा।</p>';if(!$('admin-pdf-quiz-title'))host.appendChild(title);host.appendChild(lab)}}else lab.classList.add('hidden')
}
...
function cleanCouponList(){
 ...
 rows.slice(1).forEach(x=>x.remove());const row=rows[0];if(role()==='admin'&&!row.querySelector('.rh-delete-coupon')){const code=(row.querySelector('code')?.textContent||'').trim();if(code){const actions=row.lastElementChild||row;const b=document.createElement('button');b.type='button';b.className='rh-delete-coupon rounded-xl bg-red-600 px-3 py-2 text-xs font-black text-white';b.textContent='Delete';b.onclick=()=>deleteCoupon(code,row);actions.appendChild(b)}}
}
