...
function renderBooksTab(section, body){
  body.innerHTML = `<div class="rh-focus-panel" style="padding:18px"><div class="rh-focus-cardTitle"><div><b>For each subject</b><span>Study and direct start</span></div><span>${SUBJECT_CARDS.length} subjects</span></div><div class="rh-focus-bookList">${SUBJECT_CARDS.map(r=>`<div class="rh-focus-bookRow"><div class="rh-focus-bookLeft"><div class="rh-focus-play">▶</div><div><div class="rh-focus-bookTitle">${r.name}</div><span class="rh-focus-bookSub">${r.note}</span></div></div><div class="rh-focus-inlineActions"><button type="button" class="rh-focus-inlineBtn" data-rh-book-subject="${r.name}">Start ${r.session} min</button></div></div>`).join('')}</div></div>`;
  body.querySelectorAll('[data-rh-book-subject]').forEach(btn => btn.addEventListener('click', () => { const subject = btn.getAttribute('data-rh-book-subject') || 'Biology'; const preset = SUBJECT_CARDS.find(x => x.name === subject)?.session || 50; quickSetDuration(section, preset); startFocusForSubject(section, subject); }));
}
