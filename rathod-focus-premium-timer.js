/* RATHOD HUB • VIP focus full gold layout with real data */
(function(){
'use strict';
if(window.__RH_FOCUS_TIMER_PREMIUM_V17__) return;
window.__RH_FOCUS_TIMER_PREMIUM_V17__ = 1;
...
const STORAGE_KEY = 'rh_focus_planner_tasks_v9';
const SUBJECT_CARDS = [
...
function realMembers(){ return appState.members.filter(m => Number(m.total_seconds || 0) > 0); }
...
function ensureStyle(){
  if(document.getElementById('rh-focus-vip-style-v17')) return;
  const style = document.createElement('style');
  style.id = 'rh-focus-vip-style-v17';
  style.textContent = `
    ...
    .rh-focus-vipBar{padding:18px 18px 12px;background:linear-gradient(135deg,#2a1908 0%,#4a270c 32%,#8f430e 68%,#f97316 100%);color:#fff;box-shadow:inset 0 1px 0 rgba(255,255,255,.08),0 18px 48px rgba(249,115,22,.18);position:relative}
    ...
    .rh-focus-body{padding:18px;background:linear-gradient(180deg,#111111,#0e0e0f);min-height:260px}.rh-focus-grid{display:grid;grid-template-columns:1.2fr .85fr;gap:18px}.rh-focus-split{display:grid;grid-template-columns:1fr 1fr;gap:18px}
    ...
    @media (max-width:1024px){.rh-focus-grid,.rh-focus-metrics,.rh-focus-booksGrid,.rh-focus-userGrid,.rh-focus-split{grid-template-columns:1fr}.rh-focus-vipTimer{font-size:40px}.rh-focus-graphRow{grid-template-columns:108px 1fr 52px}}
  `;
...
function currentTimerText(section){ const timerNode = allSource(section,'*').find(el => /^\d{2}:\d{2}:\d{2}$/.test(textOf(el))); return textOf(timerNode) || '00:30:00'; }
function selectedSubject(section){ const s = firstSource(section,'select'); return s?.value || s?.options?.[s.selectedIndex]?.text || 'Biology'; }
function setSubjectValue(section, subject){ const select = firstSource(section,'select'); if(!select) return false; let option = Array.from(select.options || []).find(o => String(o.value||o.textContent||'').toLowerCase() === subject.toLowerCase() || String(o.textContent||'').toLowerCase() === subject.toLowerCase()); if(!option){ option = document.createElement('option'); option.value = subject; option.textContent = subject; select.appendChild(option); } select.value = option.value; select.dispatchEvent(new Event('change', { bubbles:true })); return true; }
function quickSetDuration(section, minutes){ const input = firstSource(section,'input[type="number"]'); if(!input) return; input.value = String(minutes); input.dispatchEvent(new Event('input', { bubbles:true })); input.dispatchEvent(new Event('change', { bubbles:true })); }
function startFocusForSubject(section, subject){ setSubjectValue(section, subject); const startBtn = allSource(section,'button').find(btn => /start/i.test(textOf(btn)) && !/pomodoro/i.test(textOf(btn))); if(startBtn) startBtn.click(); appState.activeTab = 'timer'; renderApp(section); }
function getTodayStudy(section){ const todayCard = allSource(section,'div').find(el => { const t = String(el.innerText || '').toLowerCase(); return t.includes('today') && t.includes('study time'); }); const match = String(todayCard?.innerText || '').match(/(\d+h\s*\d+m|\d+h|\d+m)/i); return match ? match[1] : appState.dailyGoal; }
function getWeeklyHours(){ const members = realMembers(); const total = members.reduce((a,m)=>a+(Number(m.total_seconds||0)/3600),0); const avg = members.length ? total / members.length : 0; return [Math.max(0,avg*.4),Math.max(0,avg*.7),Math.max(0,avg*1.2),Math.max(0,avg*1.1),Math.max(0,avg*.25),Math.max(0,avg*.15),Math.max(0,avg*.1)]; }
...
function renderStudyGrid(title, subtitle, members, showRank){
  if(!members.length){
    return `<div class="rh-focus-panel" style="padding:18px"><div class="rh-focus-cardTitle"><div><b>${title}</b><span>${subtitle}</span></div><span>0</span></div><div class="rh-focus-empty">Abhi real study users nahi dikh rahe.</div></div>`;
  }
  return `<div class="rh-focus-panel" style="padding:18px"><div class="rh-focus-cardTitle"><div><b>${title}</b><span>${subtitle}</span></div><span>${members.length} users</span></div><div class="rh-focus-userGrid">${members.map((m,idx)=>`<div class="rh-focus-userCard ${idx<3?'top':''}"><div class="rh-focus-userHead"><span class="rh-focus-userName">${m.name || 'Member'}</span>${showRank?`<span class="rh-focus-rankChip">#${idx+1}</span>`:'<span class="rh-focus-liveDot"></span>'}</div><span class="rh-focus-userTime">${fmtShort(m.total_seconds || 0)}</span><span class="rh-focus-userMeta">${showRank?'VIP focus rank':'Live study time'}</span></div>`).join('')}</div></div>`;
}

function renderWeeklyPanel(){
  const vals = getWeeklyHours();
  const days = ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'];
  const max = Math.max(...vals,1);
  return `<div class="rh-focus-panel" style="padding:18px"><div class="rh-focus-cardTitle"><div><b>7 Day Focus</b><span>Real study trend style</span></div><span>${vals.reduce((a,b)=>a+b,0).toFixed(1)}h</span></div><div style="display:flex;align-items:flex-end;gap:12px;height:150px">${vals.map((v,i)=>`<div style="flex:1;display:flex;flex-direction:column;justify-content:flex-end;align-items:center;gap:8px"><div style="font-size:10px;color:#cbd5e1">${v.toFixed(1)}h</div><div style="width:100%;height:${Math.max(10,Math.round((v/max)*112))}px;border-radius:12px 12px 4px 4px;background:linear-gradient(180deg,#10b981,#059669)"></div><div style="font-size:10px;color:#9ca3af">${days[i]}</div></div>`).join('')}</div></div>`;
}
  ...
  body.innerHTML = `
    <div class="rh-focus-grid">
      <div class="rh-focus-panel rh-focus-panelOrange" style="padding:18px">
        <div class="rh-focus-cardTitle"><div><b>Focus Chamber</b><span>Gold premium live timer</span></div><span id="rh-focus-live-clock">${liveClock()}</span></div>
      ...
      <div class="rh-focus-panel" style="padding:18px">
        <div class="rh-focus-cardTitle"><div><b>VIP Controls</b><span>Fast premium controls</span></div><span>${currentTimerText(section)}</span></div>
    ...
    <div style="display:grid;gap:18px;margin-top:18px">
      ${renderStudyGrid('Live Study Members','Real users only • no fake icons', members.slice(0,6), false)}
      <div class="rh-focus-split">
        ${renderStudyGrid('VIP Focus Board','Gold ranked layout', members.slice(0,6), true)}
        ${renderWeeklyPanel()}
      </div>
    </div>`;
...
function renderInsightsTab(body){
  ...
  body.innerHTML = `
    ...
    <div style="display:grid;gap:18px">
      ${renderStudyGrid('VIP Leaderboard','Premium gold ranking', members.slice(0,6), true)}
...
function bindPlannerActions(section, body){ const input = body.querySelector('#rh-focus-planner-input'); const addBtn = body.querySelector('#rh-focus-planner-add'); const renderList = () => renderPlannerTab(section, body); addBtn?.addEventListener('click', () => { const value = String(input?.value || '').trim(); if(!value) return; const tasks = plannerStore(); tasks.unshift({ id: Date.now().toString(36), text:value, done:false, subject:selectedSubject(section), date:new Date().toISOString().slice(0,10) }); savePlanner(tasks); renderList(); }); body.querySelectorAll('[data-rh-task-toggle]').forEach(btn => btn.addEventListener('click', () => { const id = btn.getAttribute('data-rh-task-toggle'); savePlanner(plannerStore().map(t => t.id === id ? { ...t, done: !t.done } : t)); renderList(); })); body.querySelectorAll('[data-rh-task-delete]').forEach(btn => btn.addEventListener('click', () => { const id = btn.getAttribute('data-rh-task-delete'); savePlanner(plannerStore().filter(t => t.id !== id)); renderList(); })); }
function renderPlannerTab(section, body){ const subject = selectedSubject(section); const date = new Date().toISOString().slice(0,10); const tasks = plannerStore().filter(t => t.date === date); body.innerHTML = `<div class="rh-focus-panel" style="padding:18px"><div class="rh-focus-cardTitle"><div><b>${subject} Planner</b><span>User set tasks</span></div><span>${currentTimerText(section)}</span></div><div class="rh-focus-plannerInput"><input id="rh-focus-planner-input" type="text" placeholder="Aaj kya karna hai? likho..."><button id="rh-focus-planner-add" type="button">Add Task</button></div><div class="rh-focus-planList">${tasks.length?tasks.map((t,i)=>`<div class="rh-focus-planRow"><div class="rh-focus-planLeft"><div class="rh-focus-planDot">${i+1}</div><div><div class="rh-focus-planTitle ${t.done?'rh-focus-taskDone':''}">${t.text}</div><span class="rh-focus-planSub">${t.subject || subject}</span></div></div><div class="rh-focus-taskActions"><button type="button" class="rh-focus-taskChip" data-rh-task-toggle="${t.id}">${t.done?'Undo':'Done'}</button><button type="button" class="rh-focus-taskChip" data-rh-task-delete="${t.id}">Delete</button></div></div>`).join(''):'<div class="rh-focus-planRow"><div class="rh-focus-planLeft"><div class="rh-focus-planDot">+</div><div><div class="rh-focus-planTitle">No task added yet</div><span class="rh-focus-planSub">Aaj kya karna hai yaha add karo</span></div></div></div>'}</div></div>`; bindPlannerActions(section, body); }
...
function boot(){ ensureStyle(); const section = findSection(); if(!section) return; ensureSourceHost(section); ensureShell(section); renderApp(section); loadData(section); }
...
setInterval(() => { const shell = document.getElementById('rh-focus-shell'); if(!shell) return; const section = findSection(); shell.querySelector('#rh-focus-top-clock') && (shell.querySelector('#rh-focus-top-clock').textContent = liveClock()); const clockCard = document.getElementById('rh-focus-clock-card'); if(clockCard) clockCard.textContent = liveClock(); const live = document.getElementById('rh-focus-live-clock'); if(live) live.textContent = liveClock(); if(section){ shell.querySelector('#rh-focus-vip-timer') && (shell.querySelector('#rh-focus-vip-timer').textContent = currentTimerText(section)); shell.querySelector('#rh-focus-vip-subject') && (shell.querySelector('#rh-focus-vip-subject').textContent = `${selectedSubject(section)} • ${todayLabel()}`); } }, 1000);
})();
