...
function appDb(){try{if(window.db)return window.db;if(typeof db!=='undefined')return db}catch(_e){}return null}
function appUser(){try{if(window.user)return window.user;if(typeof user!=='undefined')return user}catch(_e){}return null}
function appProfile(){try{if(window.profile)return window.profile;if(typeof profile!=='undefined')return profile}catch(_e){}return null}
function email(){return String(appUser()?.email||appProfile()?.email||'').trim().toLowerCase()}
function isAdmin(){return OWNER_EMAILS.has(email())||['admin','owner'].includes(String(appProfile()?.role||'').toLowerCase())}
...
function aiHost(){
  const ids=['section-aicards','section-ai-shorts-notes','section-ai-short-notes','section-short-notes','section-aishorts','section-ainotes'];
  for(const id of ids){const node=$(id);if(node)return node}
...
function inlineCard(){
  ...
  card.querySelector('[data-rh-song-api-save]').onclick=()=>{
    const value=String(card.querySelector('[data-rh-song-api']?.value||'').trim().replace(/\/+$/,'');
...
function renderRows(rows){const card=$('rh-song-library-card');const section=librarySection();card?.querySelector('[data-rh-song-inline-list]')&&(card.querySelector('[data-rh-song-inline-list]').innerHTML=rowMarkup(rows));section?.querySelector('[data-rh-song-list]')&&(section.querySelector('[data-rh-song-list]').innerHTML=rowMarkup(rows))}
async function loadSongs(){
  const d=appDb();if(!d)return;
  inlineCard();librarySection();
...
async function uploadSong(){
  const card=$('rh-song-library-card'),d=appDb(),u=appUser(),file=card?.querySelector('[data-rh-song-file]')?.files?.[0],title=String(card?.querySelector('[data-rh-song-title]')?.value||'').trim(),status=card?.querySelector('[data-rh-song-status]');
...
function openLibrary(){
  const section=librarySection();if(!section)return;
  document.querySelectorAll('main section[id^="section-"]').forEach(node=>node.classList.add('hidden'));section.classList.remove('hidden');loadSongs();
}
function boot(){
  const card=inlineCard();librarySection();
  card?.querySelector('[data-rh-song-admin-wrap]')?.classList.toggle('hidden',!isAdmin());
  if(card)loadSongs();
}
